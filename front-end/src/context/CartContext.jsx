...
const CartProvider = ({ children }) => {
    ...
    function updateCartToDB(user_id){
        const cartUpdate = []
        cartItems.map((item)=>{
            cartUpdate.push({book_id: item.book.id, cart_item_quantity: item.cart_item_quantity})
        })
    ...
    return (
        <CartContext.Provider value={{ openCartModal, handleToggle,HandleOpenCart, HandleCloseCart, cartItems, removeBookInCart, cartQuality, addToCart, removeFromCart, getCartTotal, setCartItems, checkAllCart, updateCartToDB}}>{children}</CartContext.Provider>
    )
}
